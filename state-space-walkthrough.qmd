...
![The process of handling the state variables, the covariates, and fitting the model](./images/fig1.svg){#fig-flow}
...
# Do species harmonisation, see Schlenker et al., 2024 in refs for details
...
# There are multiple ways of coding this data-wrangling step;
# using tidyverse here for consistency with the rest of the workshop
...
Now we have all our data joined. It is always worth doing quick checks like `head()`, `tail()`, and `dim()` to catch any coding mistakes:

```{r}
#| code-fold: false
#| label: lst-check-join
...
story_pollen_matrix <- story_binned |>  # Select taxa
  ...
  rename("Fagus" = "Fagus grandifolia") |> # rename to shorten the column name
  as.matrix()
# Replacing 0 with NA is not strictly necessary for this workflow,
# but guards against treating gap rows as zero-count observations
...
Now, let's set up the parameters for `mnGLMM()`. We need:
...
Now we have reasonable starting values from `mnGLMM()`, let's set up the main state-space model parameters for `mnTS()`. `mnTS()` needs:
...
# B.start
B0.start.mnTS <- glmm_mod$B[1, , drop = FALSE] # The first row is the intercept
B.start.mnTS <- glmm_mod$B[2, , drop = FALSE] # Take care here!
# Rows 2+ are the covariates; add more rows for additional covariates
# B.start.mnTS <- glmm_mod$B[2:3, , drop = FALSE] # For 2 covariates
...
- rows = target taxon (the taxon whose abundance changes)
- columns = source taxon (the taxon exerting the effect)
- Autocorrelation on the diagonal

A positive value at row _Quercus_, column _Fagus_ means the abundance of _Fagus_ positively affects the change in abundance of _Quercus_.
...
end_time <- Sys.time()
end_time - start_time
saveRDS(mnTS_mod_refit, "./outputs/mnTS_mod_refit.rds")
```

```{r}
#| eval: false

mnTS_mod_refit <- readRDS("./outputs/mnTS_mod_refit.rds")
coef(mnTS_mod_refit)
mnTS_mod_refit$AIC
```
