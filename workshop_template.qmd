...
Now we have all our data joined. Note that the covariate $X$ (`char_acc`) has observations every cm, whereas pollen was sampled at less frequent intervals (this is OK! :smiley:). It is always worth doing quick checks like `head()`, `tail()`, and `dim()` to catch any coding mistakes:
...
Now, let's set up the parameters for `mnTS()`. `mnTS()` needs:
...
B.start.mnTS <- glmm_mod$B[2, , drop = FALSE] # Row 2 is the covariate; add rows for more covariates
...
We have found that bootstrapping provides better estimates of standard errors (and subsequent P-values). The `boot()` function will bootstrap the model, but may take a very long time. We won't do this today, but we strongly recommend bootstrapping your final models [see the full walkthrough](state-space-walkthrough.qmd).

UPDATE SECTION WITH INTERPRETATION OF COEFFICIENTS
...
::: {.callout-tip}
## Question 4a
Create new parameters for `mnGLMM()`. You can follow the code earlier in the template.
:::
...
Note the change in `glmm_mod_new_X$B[2:3, , drop = FALSE]`. With the additional X variable, the $B$ matrix now has two rows.

```{r}
#| eval: false
# B.start
B0.start.mnTS.new_X <- glmm_mod_new_X$B[1, , drop = FALSE] # The first row is the intercept
B.start.mnTS.new_X <- glmm_mod_new_X$B[2:3, , drop = FALSE] # note the change in row indices here to 2:3
